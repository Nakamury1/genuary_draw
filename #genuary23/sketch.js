// ETOILES REGULIERES
// GENUARY PROMPT 20 = Generative Architecture

let DESSIN = 7; // [7,8,9,10,11,12]

let NP = 480, PI = Math.PI;
let K = 5, H = 3, CX = NP / 2, CY = NP / 2, R = NP * 0.45, AD = PI / 2;

if (DESSIN == 8) K = 7;
else if (DESSIN == 9) K = 20, H = 9;
else if (DESSIN == 10) K = 20, H = 7;
else if (DESSIN == 11) K = 51, H = 20;
else if (DESSIN == 12) K = 51, H = 25;

function setup() {
  createCanvas(NP, NP);
  noFill();      // transparente à l'intérieur

  // carré
  stroke(100, 255); // contour du carré semi-transparent
  rectMode(CENTER);
  rect(CX, CY, NP * 0.9, NP * 0.9);

  // étoile
  stroke(230); // traits blancs
  strokeWeight(2); // optionnel, pour épaissir un peu les traits
  beginShape();
  for (let I = 0; I < K; I++) {
    let X = CX + R * cos(2 * I * H * PI / K + AD);
    let Y = CY + R * sin(2 * I * H * PI / K + AD);
    vertex(X, Y);
  }
  endShape(CLOSE);
}
